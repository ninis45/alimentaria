...
class Admin extends Admin_Controller {
	protected $section='programas';
	
	public function __construct()
	{
		parent::__construct();
        $this->lang->load('programa');
        $this->load->model(array('programa_m'));
        $this->rules = array(
    ...
    function create(){
		...
		{
			$programa->{$rule['field']} = $this->input->post($rule['field']);
		}
         $this->template->title($this->module_details['name'])            
			->set('programa',$programa)
			->build('admin/form');
